...
\subsection{Camera Field of View and Image Quality}
...
we see all the corners of the landing pad.) We thought that we would guide the
UAV to this low height using vision, and then land using dead reckoning.
However, the latter turned out to be difficult, because of airflow effects when
the UAV is close to the ground. Thus, it is critical that the UAV continue to
see the landing platform even at low heights, something our current design does
not permit. Since the UAV is rarely exactly above the landing platform, the
limited field of view is also a problem at heights above one meter.

The motion of the UAV causes some camera shake. This leads to blurry images,
especially in lower-light conditions such as a cloudy day. Figure
\ref{fig:badimage} shows an example. We were not able to extract corners from
such images.
